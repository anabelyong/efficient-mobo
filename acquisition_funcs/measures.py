"""
Code from https://github.com/yutxie/chem-measure/blob/main/measures.py:
PAPER (ICLR 2023): HOW MUCH SPACE HAS BEEN EXPLORED? MEASURING THE CHEMICAL SPACE COVERED BY 
DATABASES AND MACHINE-GENERATED MOLECULES
"""
...
import pandas as pd
from acquisition_funcs.cal_ifg_atom import CollectFG
from acquisition_funcs.cal_ringsystem import CollectRingSystems
...
def get_circles(args, silent=True):
    """
    BulkTanimotoSimilarity returns a list of floats, np.array(...) wraps that into NumPy array. 
    Numpy supports vectorized substraction 1.-np.array({}) which is element-wise.
    """
    ...
    for vec in tqdm(vecs, disable=silent):
        if len(circs) > 0:
            dists = 1. - np.array(sim_mat_func([vec], circs))
